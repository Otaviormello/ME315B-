...
```{sql connection=con}
SELECT PNAME, ACRES, (ACRES / 3) AS AREA_REDUZIDA FROM PRESERVE 
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50) / 2 AS TAXA_AJUSTADA FROM PRESERVE
```
...
COUNT(DISTINCT PNAME) AS NORMES_DISTINTOS
FROM PRESERVE;
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS TAXA FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS TOTAL_DEP FROM EMPLOYEE
```
